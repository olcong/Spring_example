...
def get_next_fingerprint_id():
    """센서에서 비어있는 가장 빠른 지문 ID를 찾습니다."""
    # 지문 센서는 보통 1부터 ID를 사용합니다.
    for finger_id in range(1, fingerprint_sensor.library_size + 1):
        if fingerprint_sensor.get_template(finger_id)['status'] != adafruit_fingerprint.OK:
            return finger_id # 비어있는 ID를 찾으면 바로 반환
    return None # 비어있는 ID가 없으면 None 반환
...
if __name__ == "__main__":
    try:
        ...
        user_id_str = input("등록할 사용자의 학번(ID)을 입력하세요: ")
        if not user_id_str.isdigit():
            raise ValueError("학번은 숫자로만 입력해야 합니다.")
